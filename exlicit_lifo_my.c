...
void *mm_realloc(void *bp, size_t size)
{
    ...
    /* 메모리가 MAX_HEAP을 초과할 때 */
    if (newp == NULL)               
    {
        return 0;
    }
    size_t oldsize = GET_SIZE(HDRP(bp));                    /* 현재 크기를 oldsize에 담음 */
    /* size가 oldsize보다 작으면 oldsize를 변경 */
    if (size < oldsize)
    {
        oldsize = size; 
    }
...
static void insert(void *ptr) {
    if (NEXT_FREE_BLKP(heap_listp) != NULL) {               /* root의 다음 가용 공간이 NULL이 아닐 때 */
        SET_NEXT_FREE(ptr, NEXT_FREE_BLKP(heap_listp));         /* 현재 ptr의 next는 루트의 next */
        SET_PREV_FREE(NEXT_FREE_BLKP(heap_listp), ptr);         /* root의 next는 ptr */
        
    } else {                                                /* root의 다음 가용 공간이 NULL일 때, 즉, 처음 들어온 가용공간 */
        SET_NEXT_FREE(ptr, NULL);                               /* ptr의 next는 NULL */
    }

    SET_NEXT_FREE(heap_listp, ptr);                         /* root의 next는 ptr */
    SET_PREV_FREE(ptr, heap_listp);                         /* ptr의 prev는 root */
}

static void delete(void *ptr) {
    void *prev = PREV_FREE_BLKP(ptr);                       /* prev는 ptr의 prev */
    void *next = NEXT_FREE_BLKP(ptr);                       /* next는 ptr의 next */
    if (NEXT_FREE_BLKP(ptr) != NULL) {                      /* ptr의 next가 NULL이 아닐 때 */
        SET_NEXT_FREE(prev, next);                              /* prev의 next는 next */
        SET_PREV_FREE(next, prev);                              /* next의 prev는 prev */
    } else {                                                /* ptr의 next가 NULL일 때, 즉, 처음 가용 공간을 지울 때 */
        SET_NEXT_FREE(PREV_FREE_BLKP(ptr), NULL);               /* ptr의 prev는 NULL */
    }
}
